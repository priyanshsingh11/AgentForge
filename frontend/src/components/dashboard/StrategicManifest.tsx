    ...
    <div className="flex flex-col gap-10 pb-20 max-w-[1400px] mx-auto animate-in fade-in slide-in-from-bottom-6 duration-1000">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 h-full">
        ...
        <div className="lg:col-span-4 glass-panel border border-white/5 bg-primary/5 rounded-[3rem] p-10 relative overflow-hidden">
          ...
          <button className="w-full py-5 bg-primary/10 border border-primary/20 rounded-2xl mt-12 text-primary font-black uppercase tracking-widest text-xs hover:bg-primary hover:text-white transition-all">
            Execute Roadmap
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-4 flex flex-col gap-6">
           ...
           <div className="space-y-4">
              {analysis.top_competitors.slice(0, 3).map((biz, i) => (
                <div key={i} className="glass-panel border border-white/5 bg-surface-container-low/30 rounded-[2rem] p-6 flex items-center justify-between group hover:bg-white/5 transition-all">
                   ...
                   <span className={`text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full ${i === 0 ? 'bg-emerald-500/10 text-emerald-500' : i === 1 ? 'bg-blue-500/10 text-blue-500' : 'bg-purple-500/10 text-purple-500'}`}>
                      {i === 0 ? 'Stable' : i === 1 ? 'Rising' : 'Disruptive'}
                   </span>
                </div>
              ))}
           </div>
        ...
        <div className="lg:col-span-4 glass-panel border border-white/5 rounded-[3rem] p-10 flex flex-col">
           ...
           <p className="text-sm text-on-surface/60 leading-relaxed mb-10 flex-1 italic">
              {strategies.gap_analysis[0]}
           </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
        <div className="lg:col-span-4 flex flex-col gap-6">
           ...
           <div className="flex-1 flex flex-col gap-4">
              {strategies.opportunities.slice(0, 3).map((opp, i) => (
